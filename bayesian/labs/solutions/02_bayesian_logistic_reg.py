...
class MetropolisSampler():
    ...
    def sample(self, size, scale=1):
        for _ in range(size):
            ...
            proposed_val = self.current_val + scale * np.random.normal(size=self.nparam)
            proposed_val_lik = self.loglik_fun(proposed_val)
            accept_prob = np.exp(proposed_val_lik - self.current_val_lik)
