...
For those who want to process the data in its very original format (where one review is one `.txt` file):
...
exp = explainer.explain_instance(
  imdb_reviews_test[test_id].decode("utf8"), rf_pipe.predict_proba, num_features=6)
...
# TODO: tf.keras.models.load_model(nn_model_file) fails??? Not related to KerasClassifier wrapper.
...
# TODO: tf.keras.models.load_model(tr_model_file) fails??? Not related to KerasClassifier wrapper.
...
explain_top_k = 1000  # Save some time.
...
if os.path.exists(lgb_model_file):
  lgbooster = lgb.Booster(model_file=lgb_model_file)
else:
  lgbooster = lgb.train(
    params=lgb_params,
...
lgb_shap_values = lgb_explainer.shap_values(X_test[:explain_top_k])
...
@nori2019interpretml open source the pacakge `interpret` for a fast implementation of **Generalized Additive Models with Pairwise Interactions** (GA<sup>2</sup>M).
...
i.e.,
models that approximate (locally) the original model for better explainability.
...
#ebm = ExplainableBoostingClassifier(n_estimators=16)
