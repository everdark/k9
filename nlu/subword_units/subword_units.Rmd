...
def viterbi_forward(word):
  """Forward step of Viterbi given a single word."""
  # Create storage array for best substring at each character position.
  ...
  for eow in range(1, len(word) + 1):
    ...
    for bow in range(eow):
      # For every possible beginning-of-word position:
      subw = line[bow:eow]
...
subw_scores, subw_slices = viterbi_forward(w)
...
subw_demo[, score:=py$subw_scores]

# To also plot the entire word by character.
w_df <- data.table(x=rep("", 7), y=0:(nrow(subw_demo)-3) + .5,
                   label=unlist(strsplit(py$w, NULL)))

ggplot(subw_demo, aes(x=subw, y=(start + end) / 2, label=subw)) +
...
print(viterbi_backward(w, subw_slices))
```
...
def viterbi(line):
  ...
  for w in words:
    subw_scores, subw_slices = viterbi_forward(w)
    subw = viterbi_backward(w, subw_slices)
    out.extend(subw)
  return out
