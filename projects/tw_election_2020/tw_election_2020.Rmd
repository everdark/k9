...
tc3_pl <- merge(tc3_pl, tc3[Candidate %in% tc3_candidates,
...
ggplot(tc3r, aes(x=reorder(Candidate, Votes), y=Votes)) +
  ...
  geom_text(aes(x=reorder(Candidate, Votes), y=Votes,
                label=sprintf("%s\n(%s)", scales::comma(Votes), scales::percent(Share))),
            size=5, hjust=-.1) +
  scale_y_continuous(labels=scales::comma, limits=c(0, max(kk$Votes) * 1.2)) +
  coord_flip()
```

```{r, fig.showtext=TRUE, fig.height=30, fig.width=6.5, fig.cap="Taichung 3rd District: Party-List Share vs Constituency Candidate Share"}
ggplot(tc3_pl, aes(x=Share, y=C_Share)) +
  geom_point(size=.5) +
  geom_smooth(method="lm") +
  labs(x="不分區政黨得票率", y="立委候選人得票率",
       title="臺中市第3選舉區不分區政黨得票率Ｘ候選人得票率") +
  #facet_grid(Party ~ Candidate, scales="free")
  facet_wrap(Party ~ Candidate, ncol=3, scales="free")
```
