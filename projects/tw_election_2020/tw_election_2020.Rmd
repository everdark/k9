...
plot_ly(tw_county_sf, split=~County, color=~Turnout, text=~paste(County, Turnout),
        showlegend=FALSE, colors="Reds", strokes="grey",
        width=800, height=600) %>%
...
plot_ly(
  p_place_l, x=~(Turnout / 100), y=~Share_Diff, text=~label, marker=list(size=3, color="black"),
...
```{r presidential_place_turnout_vs}
# This plot is a bit troublesome to optimize...
...
p1 <- plot_ly(
  p_place_l1, x=~(Turnout / 100), y=~Votes_Share, text=~label,
  color=~Candidate, colors=main_party_colors[1], marker=list(size=3),
  width=600, height=600,
  type="scatter", mode="markers") %>%
  add_lines(data=p_place_l1,
            x=~(Turnout / 100), y=fitted(fit1), color=I(main_party_colors[1]), showlegend=FALSE)
p2 <- plot_ly(
  p_place_l2, x=~(Turnout / 100), y=~Votes_Share, text=~label,
  color=~Candidate, colors=main_party_colors[2], marker=list(size=3),
  width=600, height=600,
  type="scatter", mode="markers") %>%
  add_lines(data=p_place_l2,
            x=~(Turnout / 100), y=fitted(fit2), color=I(main_party_colors[2]), showlegend=FALSE)
p3 <- plot_ly(
  p_place_l3, x=~(Turnout / 100), y=~Votes_Share, text=~label,
  color=~Candidate, colors=main_party_colors[3], marker=list(size=3),
  width=600, height=600,
  type="scatter", mode="markers") %>%
  add_lines(data=p_place_l3,
            x=~(Turnout / 100), y=fitted(fit3), color=I(main_party_colors[3]), showlegend=FALSE)
```

```{r presidential_place_turnout_vs_2, message=FALSE}
p2 %>%
  layout(title="2020台灣總統大選各投開票所：投票率 X 韓張得票率",
         yaxis=list(title="得票率", rangemode="nonnegative", scaleanchor="x"),
         xaxis=list(title="投票率", rangemode="nonnegative"),
         margin=list(t=50)) %>%
  toWebGL()
```

```{r presidential_place_turnout_vs_3, message=FALSE}
p3 %>%
  layout(title="2020台灣總統大選各投開票所：投票率 X 蔡賴得票率",
         yaxis=list(title="得票率", rangemode="nonnegative", scaleanchor="x"),
         xaxis=list(title="投票率", rangemode="nonnegative"),
         margin=list(t=50)) %>%
  toWebGL()
...
```{r presidential_place_turnout_vs_1, message=FALSE}
p1 %>%
  layout(title="2020台灣總統大選各縣市：投票率 X 宋余得票率",
         yaxis=list(title="得票率", rangemode="nonnegative", scaleanchor="x"),
         xaxis=list(title="投票率", rangemode="nonnegative"),
         margin=list(t=50)) %>%
  toWebGL()
