...
```{r presidential_county_vr70_vsd}
ggplot(p_county[Voting_Rate > 70], aes(x=Voting_Rate, y=Share_Diff, label=County)) +
  ...
  labs(x="投票率％", y="綠藍得票率差",
       title="2020台灣總統大選各縣市：投票率(>70%) X 綠藍得票率差")
```

```{r presidential_county_vr_vs, fig.width=8, fig.height=3}
...
```{r process_presidential_region_voting_rate}
...
From the previous county-level visualization we realized that voting rate may be related to the difference between DDP and KMT candidates.
...
ggplot(p_region, aes(x=Voting_Rate, y=Share_Diff)) +
  ...
  geom_smooth(method="lm") +
  labs(x="投票率％", y="綠藍得票率差",
       title="2020台灣總統大選各鄉鎮市區：投票率 X 綠藍得票率差")
```

```{r presidential_region_vr70_vsd}
ggplot(p_region[Voting_Rate > 70], aes(x=Voting_Rate, y=Share_Diff, label=Region)) +
  ...
  labs(x="投票率％", y="綠藍得票率差",
       title="2020台灣總統大選各鄉鎮市區：投票率(>70%) X 綠藍得票率差")
```

We can also test the correlation between voting rate and vote share of each candidate:
...
Interestingly,
such correlation is not found for PFP (People First Party) candidates.

```{r presidential_region_vr_vs, fig.width=8, fig.height=3}
