...
To avoid troublesome encoding issues (especially in a Windows platform with a default `CP1252` code page) the **codes in this notebook are only tested on Ubuntu and macOS.**
...
# Fix macOS ggplot device Chinese font.
theme_set(theme_get() + theme(text=element_text(family="Heiti TC Light")))
...
# Visual Analytics on Election Results {.tabset .tabset-fade .tabset-pills}
...
```{r parse_tw_county_map, class.source="fold-show"}
...
```{r process_presidential_county, class.source="fold-show", results="hide"}
...
p_county_diff[, dpp_win:=Share_Diff > 0]
...
**TODO: Adjust font (too small) or fig size.**

##### North Taiwan

```{r presidential_north_region_bar_pct_diff, fig.height=17, fig.width=8}
...
```{r presidential_central_region_bar_pct_diff, fig.height=20, fig.width=8}
plot_region_share_diff(
  central_counties,
...
```{r presidential_south_region_bar_pct_diff, fig.height=22, fig.width=8}
plot_region_share_diff(
  south_counties,
...
```{r presidential_east_region_bar_pct_diff, fig.height=6, fig.width=8}
plot_region_share_diff(
  east_counties,
...
```{r presidential_island_region_bar_pct_diff, fig.height=3, fig.width=8}
plot_region_share_diff(
  island_counties,
  title="2020台灣總統大選離島地區（鄉鎮市區）：綠藍得票率差")
```

### Invalid Vote Counts {.tabset .tabset-fade .tabset-pills}
...
p_county[, Invalid_Votes_Share:=get("無效票數B") / get("投票數C C=A+B")]
p_county[, Invalid_Votes_ShareP:=Invalid_Votes_Share * 100]
...
```{r presidential_invalid_vsd, fig.cap="2020 Presidential Election Invalid Vote Shares vs DPP-KMT Share Differences by County"}
p <- ggplot(p_county, aes(x=Invalid_Votes_Share, y=Share_Diff, label=County)) +
...
p_region[, Invalid_Votes_Share:=get("無效票數B") / get("投票數C C=A+B")]
p_region[, Invalid_Votes_ShareP:=Invalid_Votes_Share * 100]
```

```{r presidential_invalid_vsd, fig.cap="2020 Presidential Election Invalid Vote Shares vs DPP-KMT Share Differences by Region"}
...
### Voter Turnout {.tabset .tabset-fade .tabset-pills}
...
p_county_l[p_county, Turnout:=i.Turnout, on=.(County)]
```
...
```{r presidential_county_turnout_vsd}
ggplot(p_county, aes(x=Turnout, y=Share_Diff, label=County)) +
  scale_x_continuous(limits=c(35, 80)) +
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=0, ymax=Inf), fill="lightgreen", alpha=.05) +
  geom_rect(aes(xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=0), fill="lightblue", alpha=.05) +
...
```{r presidential_county_turnout70_vsd, fig.cap="2020 Presidential Election Voter Turnout (>70%) by County"}
...
```{r presidential_county_turnout_vs, fig.width=8, fig.height=3, fig.cap="2020 Presidential Election Voter Turnout vs Candidate Vote Shares by County"}
...
p_region_l[p_region, Turnout:=i.Turnout, on=.(Region)]
p_region_l[, Votes_Share:=Votes / sum(Votes), by=.(Region)]
```
...
```{r presidential_region_turnout_vsd, fig.cap="2020 Presidential Election Voter Turnout (>70%) by Region"}
...
```{r presidential_region_turnout70_vsd, fig.cap="2020 Presidential Election Voter Turnout (>70%) by Region"}
...
```{r presidential_region_vr_vs, fig.width=8, fig.height=3, fig.cap="2020 Presidential Election Voter Turnout vs Candidate Vote Shares by Region"}
...
### Voter Turnout Counts {.tabset .tabset-fade .tabset-pills}
