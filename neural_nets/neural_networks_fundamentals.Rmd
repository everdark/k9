...
The MSE loss minimization problem stated in equation $\eqref{eq:ols}$ can be rewritten as an *optimization with linear constraint*:
...
We will iterate over the coordinate space within $0 <= w1 <= 10$ and $0 <= w2 <= 10$ to collect all possible resulting loss values.
