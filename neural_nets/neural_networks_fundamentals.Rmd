...
\underbrace{ \Phi(k)(1 - \Phi(k)) }_{k = b_0 +w_0x} \cdot x_i, \\
...
\frac{\partial \mbox{MSE-Loss}}{\partial w_0}
&= - \frac{1}{N}\sum_i (y_i - \hat{y}_i) \cdot \Phi(t)(1 - \Phi(t)) \cdot 
\underbrace{\Phi(k)(1 - \Phi(k)) \cdot x_i}_{\frac{\partial z}{\partial w_0} = \frac{\partial \Phi(b0 + w_0x)}{\partial w_0}}.
\end{aligned}
$$
...
&= - \frac{1}{N} \sum_i (y_i - \hat{y}_i)\Phi(k)(1 - \Phi(k))x_i.\\
\end{aligned}
$$
...
class NerualNetworkWithMSELoss(NeuralNetwork):
  def backprop(self, lr):
    ...
    dw1 = - np.dot(self.input.T, (np.dot(self.dy*sigmoid_grad(self.output),
...
class NerualNetworkWithLogLoss(NeuralNetwork):
  def backprop(self, lr):
    ...
    dw1 = - np.dot(self.input.T, self.dy*sigmoid_grad(self.layer1))
    self.w1 -= lr * dw1
    self.w2 -= lr * dw2
```
...
X = np.array(
    [[0, 0, 1],
     ...
     [1, 1, 1]])
y = np.array([[0], [1], [1], [0]])
nn = NerualNetworkWithMSELoss(X, y)
...
X = np.array(
    [[0, 0, 1],
     ...
     [1, 1, 1]])
y = np.array([[0], [1], [1], [0]])
nn = NerualNetworkWithLogLoss(X, y)
