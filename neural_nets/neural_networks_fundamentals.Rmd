...
def sgd_optimize(X, y, grad_func, lr=.01, n_epoch=100):
  b = np.random.normal(size=X.shape[1])
  l = [loss(X, y, b)]
  out = b.copy()
...
for e in [1, 5, 10, 50, 100]:
  print("Training Epochs {:4} | Estimate: {}".format(e, sgd_beta[e]))
```
...
Here's such implementation of a batch gradient descent optimizer:

```{python linear_reg_gd_batch}
def gd_batch_optimize(X, y, grad_func, lr=.01, n_step=100, batch_size=64):
  b = np.random.normal(size=X.shape[1])
  l = [loss(X, y, b)]
  for step in range(n_step):
...
batch_beta, batch_loss = gd_batch_optimize(X, y, grad_func, n_step=3000, batch_size=64)
print(batch_beta)
```

```{python}
for bsize in [8, 16, 32, 64, 128, 256]:
  print(gd_batch_optimize(X, y, grad_func, n_step=5000, batch_size=bsize)[0])
```

```{python}
for i in range(10):
  print(gd_batch_optimize(X, y, grad_func, n_step=5000, batch_size=8)[0])
```

```{python}
for i in range(10):
  print(gd_batch_optimize(X, y, grad_func, n_step=5000, batch_size=64)[0])
```
...
$$
\operatorname*{argmin}_\beta \mbox{Loss} =
\sum_{i=1}^N\bigg( y_i - \mathrm{B}x_i\bigg)^2 + 
\lambda\bigg(\sum_{j=1}^k\vert\beta_j\vert^p\bigg).
$$
...
batch_beta, batch_loss = gd_batch_optimize(X, y, mse_grad_l2, n_step=3000, batch_size=32)
print(batch_beta)
```

```{python}
gd_optimize(X, y, mse_grad_l2)[0]
```


The resulting estimates are smaller than the vanilla gradient descent.
Let's check the result using `sklearn` (which should also give smaller estimates):

```{python ridge_reg_sklearn}
# We set fit_intercept=False since our design matrix already contains intercept.
# And according to the doc the constant 2 is not included so we set alpha = 2.
...
def mse_grad_l1(X, y, beta):
  a = 1  # Hardcode the regularization coefficient for simplicity.
  return -2 * ((y - X.dot(beta)).dot(X)) / X.shape[0] + a * np.sign(beta)# / X.shape[0]

batch_beta, batch_loss = gd_batch_optimize(X, y, mse_grad_l1, n_step=30, batch_size=1000)
print(batch_beta)
```

Again the resulting estimates are smaller than the vanilla gradient descent.
...
Let's purposely add one random feature as a noise into the design matrix and see how the regularization helps to shrink the weight of the noise.
...
```{python}
# Why our implementation can not zero out the weight?
batch_beta, batch_loss = gd_batch_optimize(Xnoise, y, mse_grad_l1, n_step=3000, batch_size=64)
print(batch_beta)

batch_beta, batch_loss = gd_batch_optimize(Xnoise, y, mse_grad_l2, n_step=3000, batch_size=64)
print(batch_beta)
```
...
## Geometric Interptretation of L1/L2 Regularization

## Probabilistic Interptretation of L1/L2 Regularization
