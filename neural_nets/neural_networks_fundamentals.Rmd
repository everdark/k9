...
Solving the above problem (using for example the method of Lagrange multipliers) will result in exactly the same target function as in equation $\eqref{eq:ols}$.
...
digraph subscript {
  ...
  x1 -> n12 [label = 'w@_{112}' color = darkgreen fontcolor = darkgreen arrowhead=none]
  x2 -> n12 [label = 'w@_{212}' color = darkgreen fontcolor = darkgreen arrowhead=none]
  x3 -> n12 [label = 'w@_{312}' color = darkgreen fontcolor = darkgreen arrowhead=none]

  n12 -> n21 [label = 'w@_{1221}' arrowhead=none]

}")
```
</div>

Notice that we purposely remove the arrorw to indicate that this is not a feed-forward mode of the graph.
Dropout is only meaningful when applied at the backpropagation stage,
i.e.,
when the weights are updated by the gradient descent or other similar optimizer.

When a neuron is dropped out,
its subsequent connection to the network are all removed.
This means the backpropagation will not update the corresponding weights all the way from the output layer to the dropped out unit.
...
This means that for each batch gradient update every neuron in the layer will have a 50% chance of being ignored before the backpropagation starts.
After dropout is applied,
a neural network model becomes thinner in a sense that it has fewer number of weights to response to the error in gradient update.
