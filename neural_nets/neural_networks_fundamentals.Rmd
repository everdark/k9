...
PYTHON_PATH <- "/usr/local/opt/python/bin/python3.7"
...
```{python linear_reg_plot, results="hide"}
from plotly.offline import plot, init_notebook_mode
import plotly.graph_objs as go

p = plot({
  "data": [go.Scatter(x=X[:,1], y=y)],
  "layout": go.Layout(title="Data Generated from Toy Model",
            xaxis=dict(title="x"),
            yaxis=dict(title="y"))
}, filename="plots/toy_reg.html", auto_open=False, include_plotlyjs=False)
```

```{r, echo=FALSE}
htmltools::includeHTML("plots/toy_reg.html")
```
...
the OLS estimator will converge in probability to the true model weights and distributed asymptotically Normal in large sample.^[One can try increase the sample size arbitrarily to see the behavior of the estimator.]
...
losses = {}
for lr in [.001, .01, .04]:
  betas[lr], losses[lr] = gd_optimize(X, y, lr=lr, n_step=10)

from pprint import pprint
pprint(betas)
```

The result suggests that a learning rate of 0.04 is already too large for our problem.
The gradient explodes which make our solution diverge.
And a rate of 0.001 converges much slower than a rate of 0.01.

This can be more clearly seen if we plot the trace of our training losses:

```{python linear_reg_diff_lr_plot}
p = plot({
  "data": [go.Scatter(x=np.arange(len(losses[.001])), y=losses[.001], name="0.001"),
           go.Scatter(x=np.arange(len(losses[.01])), y=losses[.01], name="0.01"),
           go.Scatter(x=np.arange(len(losses[.04])), y=losses[.04], name="0.04")],
  "layout": go.Layout(title="Trace of Training Loss",
            xaxis=dict(title="Step"),
            yaxis=dict(title="Loss"))
}, filename="plots/toy_reg_loss_compare.html".format(lr), auto_open=False, include_plotlyjs=False)
```

```{r, echo=FALSE}
htmltools::includeHTML("plots/toy_reg_loss_compare.html")
```
...
Checking the dynamics of loss is usually the first and quick step to examine whether the model training is functioning as expected.
...
def gd_batch_optimize(X, y, lr=.01, n_step=100, batch_size=2):
  b = np.random.normal(size=X.shape[1])
  l = [loss(X, y, b)]
  for step in range(n_step):
...
batch_beta, batch_loss = gd_batch_optimize(X, y, n_step=3000, batch_size=2)
print(batch_beta)
```
...
One epoch is for the optimizer to traverse the entire dataset once (no matter how many examples are used to calculate the gradient and do the update).
In stochastic gradient descent number of epochs can be considered another hyper-parameter of the model.
...
sgd_beta, sgd_loss = sgd_optimize(X, y, lr=.01, n_epoch=100)
print(sgd_beta)
```
...
p = plot({
  "data": [go.Scatter(x=np.arange(len(sgd_loss)), y=sgd_loss)],
  "layout": go.Layout(title="Trace of SGD Training Loss",
            xaxis=dict(title="Step"),
            yaxis=dict(title="Loss"))
}, filename="plots/toy_reg_sgd_loss.html".format(lr), auto_open=False, include_plotlyjs=False)
```

```{r, echo=FALSE}
htmltools::includeHTML("plots/toy_reg_sgd_loss.html")
```
