...
model.fit_generator(dataset, epochs=5, steps_per_epoch=1000, verbose=0)
```
...
glove.fit_generator(dataset, epochs=5, steps_per_epoch=1000, verbose=0)
```
...
For ease of a quick demo we will use the official demo sample data for `text2vec` here.
The `text8` dataset is a one long line resulting from concatenation of the entire corpus,
which is a subset of wikipedia articles.
...
Here is the command line tool output of `fasttext`:
...
# This may take some time to download. The model size is about 800+ mb.
...
((tf.reduce_sum(embeddings_4, axis=0) / tf.sqrt(4)) == embeddings).numpy().all()
```
