...
To save computation cost we will use a cached dict to maintain the word id -> ngram list mapping for quick lookups.

[TODO: handle variable length input for embedding lookup, aka sparse lookup. but keras layer does not support sparse tensor as input. need zero padding?]

ISSUES:

each line is parsed into variable length of training examples
each training example now can also have two features that are of variable length (ngram id list)
if we use flat_map due to variable length of ngrams the flat will fail unless pad with length = ngram size, which is too large

using csr matrix not working in flat_map as well.

also, Dataset.map is lazy even under eager mode,
so without flat_map we need to figure out how to handle string in tensor (py_function maybe?)

if we use sparse tensor for ngram ids? keras embedding does not support sparse tensor,
nor does it support ragged tensor.

possible workaround:
1. use tf.estimator instead of keras api so we can use tf.nn.embedding_lookup_sparse with sparse tensor
2. use sparse tensor but latter use padding within batch with dense so we can still use keras api

for 2, padding size will differ by batch but using keras input layer should be able to accept variable length input.

NOTE:
maybe try padded_batch?

how about just pad wiwht a reasonable length?
since it is just ngrams from a single word, cannot be really long.
...
```{python fasttext_vector_generator}
from tensorflow.keras.preprocessing.sequence import pad_sequences


word_vocab_size = len(word_vectorizer.vocabulary_)
sampling_table = make_sampling_table(word_vocab_size, sampling_factor=5e-3)  # Adjust dropout.
...
# Test imeplementation of embedding sparse lookup with sum as combiner in keras.
from tensorflow.keras import backend as K
...
print(K.sum(embedded * mask, axis=1))
```
...
# Combine embeddings.
target_ngram_mask = K.expand_dims(K.cast(target_ngram_embeddings._keras_mask, tf.float32))
context_ngram_mask = K.expand_dims(K.cast(context_ngram_embeddings._keras_mask, tf.float32))

# TODO:
# Using mask this way cause error in Dot layer...
target_embeddings = K.sum(target_ngram_embeddings * target_ngram_mask, axis=1, keepdims=True) + target_word_embeddings
context_embeddings = K.sum(context_ngram_embeddings * context_ngram_mask, axis=1, keepdims=True) + context_word_embeddings
...
model = tf.keras.Model(inputs=inputs, outputs=outputs, name="fastText")
model.compile(loss="binary_crossentropy", optimizer="sgd")
