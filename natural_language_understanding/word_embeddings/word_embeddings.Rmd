...
we use `sentencepiece` (@kudo2018sentencepiece) to learn subwords from Shakespeare.^[For more details on subword unit segmentation, reader can refer to the [notebook of subword units](https://everdark.github.io/k9/natural_language_understanding/subword_units/subword_units.nb.html).]
...
for i, (x, y) in enumerate(zip(pairs, labels)):
  if i == 0:
    print("{:6} -> {:10} | {}{}".format("target", "context", "label", sep))
  target, context = [sp.IdToPiece(i).replace("\u2581", "") for i in x]
  print("{:6} -> {:10} | {}".format(target, context, y))
```
...
The formula follows exactly equqtion $\eqref{eq:dropout}$,
...
Based on the [source code](https://github.com/keras-team/keras-preprocessing/blob/master/keras_preprocessing/sequence.py) of `keras.preprocessing.sequence.skipgrams`,
...
The main difference lies in how we prepare the training examples.^[For more on the matrix factorization problem, especially for application in recommender systems, one can refer to the [notebook of Matrix Factorization](https://everdark.github.io/k9/matrix_factorization/matrix_factorization.nb.html).]

```{python word2vec_tf_model}
# Dimension of embeddings. Typically 128 to 512.
# For our toy example we choose a smaller number.
k = 32
...
model.fit(dataset, epochs=10, steps_per_epoch=10000, verbose=0)
```
