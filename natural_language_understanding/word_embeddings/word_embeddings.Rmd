...
This notebook is written with [`reticulate`](https://github.com/rstudio/reticulate)(@reticulate),
...
plot(freq_to_drop(rank_to_freq(1:length(py$sampling_table)), t=1e-3), type="l",
...
optimizer = tf.keras.optimizers.SGD(lr=.05, decay=1e-6, momentum=.9)
...
Even only 10k sentences can easily expand to millions of training examples depending on the setup of negative sampling and context window.

For such simple factorization there are ways to optimize but won't fit into the high-level APIs we used.
We will demonstrate in latter section how fast the training can be achieved should we choose to use a library with specific optimization done for this model.
But as of now,
...
model.fit(x=[x[:,0], x[:,1]], y=y, batch_size=2048, epochs=5, verbose=1)
```
...
without actually traversing through the entire corpus we won't know how many training examples are actually generated from out corpus.
...
```python
model.fit_generator(dataset, epochs=1, steps_per_epoch=5000, verbose=0)
```

But it seems that the gradient only get updates per epoch instead of per batch.
(Not sure if it has something to do with the inter-play between `tf.data.Dataset` and `tf.keras.Model`, the latter definitely has better support for its own implementation of data iterator which is of class `tf.keras.utils.Sequence`.)
For our model the training time will hence become prohibitively too long.
To workaround this,
let's use `train_on_batch` method instead.
In this way we need to write our own training loop code:

```{python w2v_tf_model_train_on_batch}
...
for i, (x, y) in enumerate(dataset):
  if i < n_steps:
    loss = model.train_on_batch(x, y, reset_metrics=False)
    if i % 1000 == 0:
...
As one can see,
for such a the small corpus the model quickly converges.
The learned embeddings can be retrieved from a layer of our neural nets.
It is simply a matrix of dimension with vocab size by embedding size.
...
Also,
not too surprise,
by using a framework the training speed is much faster than our simple educational implementation in tensorflow.
...
# We train only 1 epoch to save time.
glove.fit_generator(dataset, epochs=1, steps_per_epoch=100000, verbose=0)
```
...
One should also realize that the training speed is considerably faster with `text2vec` (even with a larger vocabulary size) than our previous hand-crafted `tensorflow` implementation.
This is totally expected since our implementation is really for educational purpose and is not optimized at all.
