...
user_embeddings = tf.keras.layers.Embedding(
  input_dim=m, output_dim=k, name="user_embedding")(user_inputs)
item_embeddings = tf.keras.layers.Embedding(
  input_dim=n, output_dim=k, name="item_embedding")(item_inputs)
dots = tf.keras.layers.Dot(axes=-1, name="logits")([user_embeddings, item_embeddings])
model = tf.keras.Model(inputs=[user_inputs, item_inputs],
                       outputs=dots,
...
```{python keras_fit}
model.fit(train_data, epochs=10, steps_per_epoch=100, verbose=0)
```
...
keras_preds = model.predict({"user": R_u, "item": R_i})
...
keras_user_embeddings = model.get_layer(name="user_embedding").weights[0].numpy()
keras_item_embeddings = model.get_layer(name="item_embedding").weights[0].numpy()
print(np.round(keras_user_embeddings.dot(keras_item_embeddings.T), 2))
```
