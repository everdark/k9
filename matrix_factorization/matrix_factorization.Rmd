...
A common choice for $P(i \succ j | \Theta)$ will be a logistic function $\sigma(t) = \frac{1}{1 + e^{-t}}$, where the logits $t$ can be any linear or non-linear function in $\Theta$.
...
with the regularizer being the size of the variance of each parameter prior.^[Again for a detailed discussion on this, one can refer to [this notebook](file:///D:/Users/fcdyuc/projects/k9/neural_nets/neural_networks_fundamentals.nb.html#44_probabilistic_interpretation_of_l1l2_regularization).]
...
where the embeddings are the model weights to be learned with a BPR loss minimization.
