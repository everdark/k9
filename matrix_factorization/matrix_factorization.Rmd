...
In order to implement the solver we need to manually derive the gradient:^[Here we use the common derivatives: $\frac{d\ln f(x)}{dx} = \frac{f'(x)}{f(x)}$ and $\frac{de^{f(x)}}{dx} = f'(x)e^{f(x)}.$]

$$
\frac{\partial L(p_u, q_i, r_{ui})}{\partial p_{u1}} =
- \sum_{i} \bigg[
\alpha r_{ui}q_{i1} - \frac{(1 + \alpha r_{ui} q_{i1})e^{p_u^Tq_i}}{1 + e^{p_u^Tq_i}}
\bigg].
$$
