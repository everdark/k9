...
where $\phi_{ui}$ is again a binary indicator of user $u$ ever interacted with item $i$.
...
- \sum_{i} \bigg[
\alpha r_{ui}q_{ik} - \frac{(1 + \alpha r_{ui} q_{ik})e^{p_u^Tq_i}}{1 + e^{p_u^Tq_i}}
\bigg].
$$
...
- \sum_{i} \bigg[
\alpha r_{ui}q_{i} - \frac{(1 + \alpha r_{ui} q_{i})e^{p_u^Tq_i}}{1 + e^{p_u^Tq_i}}
\bigg].
$$

```{python logistic_mf}
class LogisticMF:
  def __init__(self, R, k=3, a=None, lambd=.0001):
    self.R = R
    self.k = k
    self.lambd = lambd
    self.m, self.n = R.shape
  ...
  def train(self, lr=.01, n_step=10):
    ...
    for step in range(n_step):
      ...
      # Fix P and update Q:
      for i in range(self.n):
        arp = self.a * self.R[:,i][:, np.newaxis] * self.P
        epq = np.exp(self.P.dot(self.Q[i]).sum())
        grads = (- arp + ((1 + arp) * epq) / (1 + epq)).sum(axis=0) + self.lambd * self.Q[i]
        self.Q[i] -= lr * grads
      # Fix Q and update P:
      for u in range(self.m):
        arq = self.a * self.R[u,:][:, np.newaxis] * self.Q
        epq = np.exp(self.Q.dot(self.P[u]).sum())
        grads = (- arq + ((1 + arq) * epq) / (1 + epq)).sum(axis=0) + self.lambd * self.P[u]
        self.P[u] -= lr * grads
      # Trace the loss per step.
      logits = self._sigmoid(self.P.dot(self.Q.T))
      loglik_p1 = (self.a * self.R[self.R.nonzero()] * logits[self.R.nonzero()]).sum()
      loglik_p0 = logits[np.where(self.R == 0)].sum()
      loglik = loglik_p1 + loglik_p0
      _l2 = (pow(self.P, 2).sum() + pow(self.Q, 2).sum())
...
lmf_model = LogisticMF(ratings)
lmf_model.train(n_step=50)
print(np.round(lmf_model.predict_all(), 2))
```

```{python logistic_mf_loss}
for i, loss in enumerate(lmf_model.loss):
  print("Training Step {:2} | Loss: {}".format(i + 1, np.round(loss, 4)))
```
...
[Wrong results. Implementation needs debug.]

### Bayesian Personalized Ranking

Contrary to the point-wise (per user-item pair) loss framework discussed above,
@rendle2009bpr proposed the idea of a pair-wise loss optimization approach called the BPR model which is also widely adopted in many recommender system algorithms.

[Discussion goes here.]

[Toy implementation goes here.]
...
to_file(ratings, "R.txt")
to_file(b_ratings, "Rb.txt")
```
...
mf-train -f 0 -k 3 -l2 .0001 -t 100 R.txt models/libmf_rvmf.model > $LOSS
...
mf-train -f 5 -k 3 -l2 .0001 -t 100 Rb.txt models/libmf_bmf.model > $LOSS
...
Finally,
for all supported features of `libmf`:
...
[`Starspace`](https://github.com/facebookresearch/Starspace.git)(wu2017starspace) is a library developed by FaceBook AI Research as a general factorization framework for a variety kinds of machine learning task.
