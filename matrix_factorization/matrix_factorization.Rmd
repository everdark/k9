...
The technique can also be used to train an explicit feedback model,
...
when we fix item embeddings $q_i$ as constant,
the problem of finding the optimal user embedding $p_u$ for user $u$ actually reduces to *a linear regression problem given the user for all items*!
...
$$
\hat{p_u} = (Q'C_uQ)^{-1}Q'C_u\phi_u,
$$

where $C_u$ is a diagonal matrix
...
\hat{p_u} = (Q'C_uQ + \lambda I)^{-1}Q'C_u\phi_u.
\end{equation}
$$

That is,
in each step of the alternating least squares fixing item embeddings,
our solution to each user's embedding vector has a closed form.
...
\bigg] + \lambda\bigg(\sum_up_u^2 + \sum_iq_i^2\bigg).
\end{equation}
$$
...
=
\begin{bmatrix}
-2 \sum_{i=1}^m c_{u1}(\phi_{u1} - p_{u1}q_{i1})q_{i1} + 2\lambda
\vphantom{\frac{\partial L()}{\partial p_{u1}}} \\
-2 \sum_{i=1}^m c_{u2}(\phi_{u2} - p_{u2}q_{i2})q_{i2} + 2\lambda
\vphantom{\frac{\partial L()}{\partial p_{u1}}} \\
\vdots \\
-2 \sum_{i=1}^m c_{uk}(\phi_{uk} - p_{uk}q_{ik})q_{ik} + 2\lambda
\vphantom{\frac{\partial L()}{\partial p_{u1}}} \\
\end{bmatrix}.
$$

Setting all the gradients to zero (a.k.a first-order condition) gives the optimal solution for $\hat{p_{uk}}$ as:

$$
\begin{bmatrix}
\sum_{i=1}^mc_{u1}(\phi_{u1} - \hat{p_{u1}}q_{i1})q_{i1} \\
\sum_{i=1}^mc_{u2}(\phi_{u2} - \hat{p_{u2}}q_{i2})q_{i2} \\
\vdots \\
\sum_{i=1}^mc_{uk}(\phi_{uk} - \hat{p_{uk}}q_{ik})q_{ik}
\end{bmatrix}
=
\begin{bmatrix}
\lambda \vphantom{\sum_{i=1}^m} \\
\lambda \vphantom{\sum_{i=1}^m} \\
\vdots \\
\lambda \vphantom{\sum_{i=1}^m} \\
\end{bmatrix}.
...
$$
\begin{bmatrix}
\hat{p_{u1}} \vphantom{\frac{c_{u1}\phi_{u1}\sum_iq_{i1} - \lambda}{c_{u1}\sum_iq_{i1}^2}} \\
\hat{p_{u2}} \vphantom{\frac{c_{u1}\phi_{u1}\sum_iq_{i1} - \lambda}{c_{u1}\sum_iq_{i1}^2}} \\
\vdots \\
\hat{p_{uk}} \vphantom{\frac{c_{u1}\phi_{u1}\sum_iq_{i1} - \lambda}{c_{u1}\sum_iq_{i1}^2}} \\
\end{bmatrix}
=
\begin{bmatrix}
\frac{c_{u1}\phi_{u1}\sum_iq_{i1} - \lambda}{c_{u1}\sum_iq_{i1}^2} \\
\frac{c_{u2}\phi_{u2}\sum_iq_{i2} - \lambda}{c_{u2}\sum_iq_{i2}^2} \\
\vdots \\
\frac{c_{uk}\phi_{uk}\sum_iq_{ik} - \lambda}{c_{uk}\sum_iq_{ik}^2}
\end{bmatrix}.
$$

Now if we express the vector in matrix notation,
we will arrive at exactly equation $\eqref{eq:als}$.

### Logistic Matrix Factorization {-}

We can also replace the MSE loss with a cross entropy,
the optimization problem then becomes a probablistic model.
...
@rendle2009bpr proposed the idea of a pair-wise loss optimization approach called the BPR loss which is also widely adopted in many recommender system framework.
