...
One implication of this is that we can customize the default confidence level (in our toy example the default is 1 for all user-item paris) for users who we believe have a certain focus of interests.
For example for users who mostly browse fashion products.
By setting a higher default confidence level of these users on fashion products,
the recommeder system will also shift towards recommeding more fashion products to these users.
...
which is equivalent to minimize the log-likelihood:^[Here we follow the notation of @johnson2014logistic to denote $Pr(\phi_{ui} = 1) = \frac{e^{p_u^Tq_i}}{1 + e^{p_u^Tq_i}}$. It is also common to write $Pr(\phi_{ui} = 1) = \frac{1}{1 + e^{-p_u^Tq_i}}$ as what we did earlier in this section.]

$$
\begin{aligned}
\min_{P, Q} L(p_u, q_i, r_{ui})
&= \sum_{u, i \in R} \bigg[
\alpha r_{ui}\log Pr(\phi_{ui} = 1) + \log Pr(\phi_{ui} = 0)
\bigg] \\
&=\sum_{u, i \in R} \bigg[
\alpha r_{ui}p_u^Tq_i + (1 + \alpha r_{ui})\log(1 + e^{p_u^Tq_i})
\bigg].
\end{aligned}
$$
...
Contrary to the point-wise loss framework discussed above,
