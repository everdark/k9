...
In the rest of the notebook we will use the following notations for user embedding matrix $P$ and item embedding matrix $Q$:
...
And for embedding vector $p_u$ and $q_i$ representing individual user $u$ and item $i$:
...
To learn the all these weights,
...
But we can initialize the embeddings with random weights and apply numerical method such as *stochastic gradient descent* to approximate the solution.
...
Fortunately the algorithm is [*embarrassingly parallel*](https://en.wikipedia.org/wiki/Embarrassingly_parallel),
meaning that we can improve the performance considerably by parallel computing.
