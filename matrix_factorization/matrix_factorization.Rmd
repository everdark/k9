...
3. Value of a feedback can be viewed as *confidence* rather than *preference*.
4. Evaluation must be handled properly. (More on this latter.)
...
Indeed a matrix factorization model *is* a shallow neural network model.^[More a overall discussion about neural networks one can refer to [this notebook](https://everdark.github.io/k9/neural_nets/neural_networks_fundamentals.nb.html).]

<div class="fold s">
```{r mf_as_nn_diagram, fig.width=6, fig.height=3}
DiagrammeR::grViz("
digraph subscript {
  labelloc='t'
...
In this section we will discuss several high-quality open-sourced libraries designed for factorization model.
...
In [Apache Spark](https://spark.apache.org/) the [MlLib module](https://spark.apache.org/mllib/) as a [Collaborative Filtering submodule](https://spark.apache.org/docs/latest/ml-collaborative-filtering.html) which implements the matrix factorization on both explicit and implicit feedback assumption using *alternating least squares* as the solver.

Since Spark is designed for distributed computing it can handle large scale application quite well in practice.
