...
\hat{p_u} = (Q'C_uQ + \lambda_p I)^{-1}Q'C_u\phi_u.
\end{equation}
$$
...
Setting all the gradients to zero (a.k.a first-order condition) gives the optimal solution for $\hat{p_{u}}$ as:
...
\end{bmatrix}
}_{Q'C_uQ\hat{p_u} + \lambda I}.
$$

Now if we express the vector in matrix notation,
we will arrive exactly at equation $\eqref{eq:als}$.

Notice that a speedup can be achieved by the fact that

$$
Q'C_uQ, = Q'Q + Q'(C_u - I)Q.
$$

This is because $C_u$ varies by user but $Q'Q$ is the same for all users.
When iterate over users we only need to compute $Q'Q$ once and also $C_u - I$ is sparse so it can reduce computation further.
...
since each user (and each item) embedding vector can be solved by a linear regression model separately.
