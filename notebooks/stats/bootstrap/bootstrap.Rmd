...
- name: Kyle Chung
  affiliation:
date: "`r format(Sys.time(), '%d %b %Y')` Last Updated (08 Jan 2020 First Uploaded)"
output:
  html_notebook:
    highlight: tango
    number_sections: yes
...
In this section we further discuss its application for a linear regressor.
...
We fit the model and report the heteroskedasticity consistent standard error:
...
```{r boot_lm_casewise_point}
rowMeans(bs_coefs)
```

```{r boot_lm_casewise_sd}
apply(bs_coefs, 1, sd)
```
...
for ( i in seq_len(M) ) {
  ...
  y_b <- yhat + e_b
  bs_coefs2[,i] <- ols(X=X, y=y_b)
}
```

```{r boot_lm_resid_point}
rowMeans(bs_coefs2)
```

```{r boot_lm_resid_sd}
apply(bs_coefs2, 1, sd)
```
...
1. Min and max statistic (end-point estimator)
2. population with infinite variance

The second item is easier to understand,
if we know that the proof of the convergence of bootstrap distribution toward sampling distribution is done by using CLT.
Remember that finite variance is one requirement for CLT to apply.
