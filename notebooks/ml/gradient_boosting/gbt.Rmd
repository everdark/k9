...
For convenience let's re-state the model function here,
...
`xgboost` approximates the loss with a 2nd-order [Taylor series]((https://en.wikipedia.org/wiki/Taylor_series)) expansion:

$$\begin{aligned}
\text{Approx. Loss} &= \sum_{i=1}^n L(y_i, \hat{y_i}^{(k)}) \\
&= \sum_{i=1}^n L(y_i, \hat{y_i}^{(k-1)} + f_k(x_i)).
\end{aligned}
$$
