...
dy <- eval(dy2y)
D[!is_test, g:=attr(dy, "gradient")[,"yhat"]]
D[!is_test, h:=attr(dy, "hessian")[,"yhat","yhat"]]
...
bst3 <- xgb.train(params=list(objective="reg:squarederror", max_depth=1),
                  data=Dx, nround=1)
print(xgb.model.dt.tree(model=bst3))
```
...
Instead of splitting at raw feature values histogream-based approach bucketizes continous features into discrete bins,
...
Histogram tree cannot be optimized for sparse data though.
...
^[Also, histogram tree is not particularly novel in `lightgbm`.
...
https://github.com/Microsoft/LightGBM/blob/7a89d0054bcf0db9f96af3c53a078b6aa3152efe/src/io/bin.cpp#L207-L401
...
One such strategy is called Gradient-Based One-Side Sampling (GOSS).
...
For sparse data `lightgbm` adopts a strategy to group together features that rarely co-exist into single feature and do the split scan.
This is referred to as EFB.
GOSS handles long (number of examples) data while EFB handles the wide part (number of features).
...
print(lgb.model.dt.tree(model=lgbst2)[])
```

# XGBoost v.s. LightGBM

`xgboost` and `lightgbm` have a lot in common,
even their APIs are highly similar.
...
They handle missing values in the same way.
They both support training a random forest instead of boosting trees.
^[While `xgboost` allows training a boosted RF (that is, RF as base learner),
`lightgbm` only supports training a regular RF.]
...
One remaining difference is `lightgbm`'s special support for categorical features *without* one-hot-encoding.
For training data containing categoricals the usual way to do is to one-hot encode the data into a sparse matrix.
But for `lightgbm` such effort is saved so the data pipeline can be much cleaner.

```{r}
# TODO: DEMO categorical features.
```
