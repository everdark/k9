...
hd_rf = RandomForestClassifier(n_estimators=100)
_ = hd_rf.fit(X_train, y_train)

print(classification_report(y_test, hd_rf.predict(X_test)))
print(roc_auc_score(y_test, hd_rf.predict_proba(X_test)[:,1]))
```
...
rf_tab_explainer = LimeTabularExplainer(
  X_train.values, class_names=["Negative", "Positive"],
  feature_names=ucihd_2.columns,
  categorical_features=cat_ind)

tp_idx = np.where(np.logical_and(hd_rf.predict(X_test) == 1, y_test == 1))[0]
fp_idx = np.where(np.logical_and(hd_rf.predict(X_test) == 1, y_test == 0))[0]

# We take one true positive and one false positive for examples.
tp_exp = rf_tab_explainer.explain_instance(
  X_test.iloc[tp_idx[0]], hd_rf.predict_proba, num_features=4)
fp_exp = rf_tab_explainer.explain_instance(
  X_test.iloc[fp_idx[0]], hd_rf.predict_proba, num_features=4)
tp_exp.save_to_file("/tmp/explain_tab_rf_tp.html")
fp_exp.save_to_file("/tmp/explain_tab_rf_fp.html")
```

#### A True Positive Prediction Explained {-}

```{r, echo=FALSE}
htmltools::includeHTML("/tmp/explain_tab_rf_tp.html")
```

#### A False Positive Prediction Explained {-}

```{r, echo=FALSE}
htmltools::includeHTML("/tmp/explain_tab_rf_fp.html")
```
...
ucihd_tr = lgb.Dataset(X_train, label=y_train)
ucihd_te = lgb.Dataset(X_test, label=y_test)
...
hd_bst = lgb.train(
  params=hd_lgb_params,
  num_boost_round=300, early_stopping_rounds=20,
  train_set=ucihd_tr, valid_sets=[ucihd_te],
  verbose_eval=5)

hd_lgb_yhat = hd_bst.predict(ucihd_test.iloc[:,:-1])
hd_lgb_pred = (hd_lgb_yhat > .5).astype(int)
print(classification_report(y_test, hd_lgb_pred))
print(roc_auc_score(y_test, hd_lgb_yhat))
```

In this particular (rather small) dataset RF indeed outperforms GBT.
As a matter of fact,
a simple logistic regression may have a even higher score for this problem.
Nevertheless, let's move on to our explanation model with LIME:

```{python lime_tabular_gbt}
def lgb_predict_fn(x):
  # We need to output 2 columns for binary prob prediction.
  p = hd_bst_2.predict(x).reshape(-1, 1)
  return np.hstack((1 - p, p))

lgb_tab_explainer = LimeTabularExplainer(
  X_train.values, class_names=["Negative", "Positive"],
  feature_names=ucihd_2.columns,
  categorical_features=cat_ind)

# We take the same examples previously explained in our RF explanation model.
tp_exp = lgb_tab_explainer.explain_instance(
  X_test.iloc[tp_idx[0]], lgb_predict_fn, num_features=4)
fp_exp = lgb_tab_explainer.explain_instance(
  X_test.iloc[fp_idx[0]], lgb_predict_fn, num_features=4)
tp_exp.save_to_file("/tmp/explain_tab_lgb_tp.html")
fp_exp.save_to_file("/tmp/explain_tab_lgb_fp.html")
```

```{r, echo=FALSE}
htmltools::includeHTML("/tmp/explain_tab_lgb_tp.html")
```

```{r, echo=FALSE}
htmltools::includeHTML("/tmp/explain_tab_lgb_fp.html")
```
...
ucihd_tr = lgb.Dataset(ucihd_train.iloc[:,:-1], label=ucihd_train["label"],
                       categorical_feature=categorical_attr,
                       free_raw_data=False)
ucihd_te = lgb.Dataset(ucihd_test.iloc[:,:-1], label=ucihd_test["label"],
                       categorical_feature=categorical_attr,
                       free_raw_data=False)
...
hd_lgb_yhat = hd_bst.predict(ucihd_test.iloc[:,:-1])
hd_lgb_pred = (hd_lgb_yhat > .5).astype(int)
print(classification_report(ucihd_test["label"], hd_lgb_pred))
print(roc_auc_score(ucihd_test["label"], hd_lgb_yhat))
```
