...
- name: Kyle Chung
  affiliation:
date: "`r format(Sys.time(), '%d %b %Y')` Last Updated (08 Dec 2019 First Uploaded)"
output:
  html_notebook:
    highlight: tango
    number_sections: yes
...
```{python text_tfidf}
imdb_vectorizer = TfidfVectorizer(lowercase=True, min_df=10)
...
if os.path.exists(imdb_rf_model_file):
  ...
  imdb_rf = RandomForestClassifier(n_estimators=300)
  _ = imdb_rf.fit(imdb_X_train, imdb_y_train)
  _ = joblib.dump(rf, imdb_rf_model_file)
...
nn_yhat = np.squeeze(nn_model.predict(seq_test_padded))
nn_pred = (nn_yhat > .5).astype(int)

print(classification_report(imdb_y_test, nn_pred))
print(roc_auc_score(imdb_y_test, nn_pred))
```
...
imdb_tr_yhat = np.squeeze(tr_model.predict(seq_test_padded))
...
ucihd_rf = RandomForestClassifier(n_estimators=100)
...
imdb_X_test_d = imdb_X_test.toarray()
...
ax = rf_feat_imp.tail(top_k).plot(kind="barh")
plt.show()
```
...
imdb_tr = lgb.Dataset(imdb_X_train, label=imdb_y_train, feature_name=sorted_vocab_ascii)
imdb_te = lgb.Dataset(imdb_X_test, label=imdb_y_test, feature_name=sorted_vocab_ascii)
...
if os.path.exists(imdb_lgb_model_file):
  ...
  imdb_bst = lgb.train(
    ...
    train_set=imdb_tr, valid_sets=[imdb_te],
    verbose_eval=100)
...
def imdb_lgb_shap_plot(test_id):
...
imdb_lgb_tp_shap = imdb_lgb_shap_plot(imdb_rf_tp_idx[0])
imdb_lgb_fp_shap = imdb_lgb_shap_plot(imdb_rf_fp_idx[0])

shap.save_html("/tmp/imdb_lgb_shap_tp.html", imdb_lgb_tp_shap)
shap.save_html("/tmp/imdb_lgb_shap_fp.html", imdb_lgb_fp_shap)
```

```{r, echo=FALSE}
htmltools::includeHTML("/tmp/imdb_lgb_shap_tp.html")
htmltools::includeHTML("/tmp/imdb_lgb_shap_fp.html")
```
...
# Global importance view on the fisrt feature.
ucihd_ebm_global.visualize(0).write_html("/tmp/ucihd_ebm_age_imp.html", include_plotlyjs=False)

#ucihd_ebm_global.visualize().write_image("test.svg")  # Need ploty/orca.
```
