...
meta <- c(
    ...
    '<meta property="og:url" content="https://everdark.github.io/k9/notebooks/ml/natural_language_understanding/word_embeddings/word_embeddings.nb.html">',
...
github_corner_conf <- list(github_link="https://github.com/everdark/k9/tree/master/notebooks/ml/natural_language_understanding/word_embeddings")
...
^[Windows user may encounter encoding issues using the script.
A quick fix is to replace the line
`input = fileinput.FileInput(input_file, openhook=fileinput.hook_compressed)` with
...
# For acutal application shuffle is a must since otherwise it will bias the SGD optimizer.
...
The main difference lies in just how we prepare the training examples.
^[For more on the matrix factorization problem, especially for application in recommender systems,
one can refer to the [notebook: Matrix Factorization for Recommender Systems](https://everdark.github.io/k9/notebooks/ml/matrix_factorization/matrix_factorization.nb.html).]
...
It turns out that this helps the model learn meaningful embeddings faster.
...
Or to save memory footprint we can train by sentence (hence with varying batch size),
...
To train the model,
ideally we can simply call the `fit_generator` method on our `tf.data.Dataset`,
just like this:
^[As of the notebook is written,
`fit()` is buggy in TF 2.0.0 stable release (but not in the 2.0.0 beta),
so we use `fit_generator()` instead.]
...
tf_w2v_model_file = "models/enwiki_w2v.h5"

# Save/reload the model to save notebook rendering time.
if os.path.exists(tf_w2v_model_file):
...
glove_model_file = "models/enwiki_glove.h5"
if os.path.exists(glove_model_file):
  # Model with custom objects cannot be directly loaded with compiliation.
  # We need too compile with the custom objects AFTER loaded.
  glove = tf.keras.models.load_model(glove_model_file, compile=False)
  glove.compile(loss=weighted_sum_squared_error, optimizer="sgd")
else:
  n_epoch = 5
