...
class(f)  # A MultiprocessFuture object.
```
...
class(futureOf(v))  # A Future object.
```
...
class(fe)  # No exception before access the value.
...
The esign philosophy is that we never know when the `promise`d value will be available,
