...
subtitle: "From `future` to `promises` then a Scalable `shiny` App"
author:
- name: Kyle Chung
  affiliation:
...
nocite: | 
  ...
  @shiny
abstract: |
  The notebook is a practical walk-through of implementing asynchronous programming in R using package `future` and `promises`, mainly for the purpose of developing a scalable `shiny` application. Indeed, all 3 packages come with very well-structured official tutorials already. This notebook serves more as a one-stop reference for developers to quickly hands-on on the topic and get ready for the actual application development.
---
...
github_corner_conf <- list(github_link="https://github.com/everdark/k9/tree/master/programming/async")
...
By default `future` will automatically scan the given expression and determine which variable to copy for access.
...
which in theory can result in error by overlooking variable required by `expr`.
The search includes the `envir` (by default is the calling `environment`) AND its parents until found (a.k.a. *lexical scoping*):
...
#### Search Scope Inconsistency {-}

[behavior difference in multisession and multicore, only multisession has this inconsistency]
...
One tricky finding is that the behavior can differ for different execution plans.
Indeed under a `sequential` plan the code works just fine:
...
```{r future_global_copy_multiprocess}
plan(multiprocess)
...
But for a `multiprocess` plan:

```{r future_scope_superassign_multiprocess}
plan(multiprocess)
x <- 0
f <- future({
  x <<- 42
...
Put it differently,
`resolved` only works for `Future` object but not the `promise` object.
...
print(p)  # Now should be fulfilled. (If not, just wait longer.)
```
...
If for some reason we definitely want to access the bounded value from a `promise`,
a workaround is to use super-assignment:

[TODO: This is not working under notebook calling environment.]

```{r promise_super_assign}
gv <- NULL
...
Remeber that in `future` expression the calling environment is kept intact even with super-assignment.
This is not true for `promise`.
For the `promise` programming API we do have the ability to export any value to the calling environment.
Such trick,
however,
must be used with high caution (as it is for any operation involving super-assignment).

## Error Handling

When a `then` job failed at its `onFulfilled` task,
...
# The first promise failed at its onFuilfilled task.
p1 <- then(
  f,
  onFulfilled=function(v) stop(v),
...
p2 <- then(
  ...
  onRejected=function(e) print(sprintf("From onRejected: %s", e))
)
...
f <- future({
  ...
  cat(.)  # This won't show on the code chunk output.
}
```
...
Reactive programming in R is introduced by the well-known `shiny` package which facilitates the ease of web application development purely using R,
...
To register an object whose value changes reactively (according to, say, user input from the web app portal),
...
ui <- fluidPage(
  ...
  fluidRow(
    column(6, actionButton("do", "Do something.")),
    column(6, uiOutput("out"))
  )
