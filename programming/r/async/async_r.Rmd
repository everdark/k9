...
All reactive functions provided by `shiny` are both `future` and `promises` aware.
It means that reactive expressions can be a `Future` or a `promise`:


```{r async_app, eval=FALSE}
...
And navigate to `127.0.0.1:8787`.
Open multiple tabs and hit the button sequentially to see if the performance of the app in one tab is affected by the other under different execution plans.

[try shinyloadtest?]

[invalidateLater]

[super assign in promise]
